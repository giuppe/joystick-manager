...
from Observer import EventSubject



import fcntl, struct

class Joystick(EventSubject):
    ...
    def update(self):
        ...
        if(etype & self.JS_EVENT_AXIS):
            ...
            self.sendSignal(self.JS_EVENT_AXIS, enumber, evalue)
        elif(etype & self.JS_EVENT_BUTTON):
            print "%s: Button %d, value %d" % (etime, enumber, evalue)
            self.sendSignal(self.JS_EVENT_BUTTON, enumber, evalue)
        else:     
            print etime
            print evalue
            print etype
